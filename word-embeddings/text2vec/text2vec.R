...
clean_data_for_word_embeddings <- function(data_decade_subset) {
  ...
  #  filter(!str_detect(ngram, "[[:digit:]]"))
  
  data_decade_subset$ngram <- str_replace(data_decade_subset$ngram, "'s", "")
  
  stopwords <- import_stopwords_as_regex()
  
  data_decade_subset <- data_decade_subset %>%
    filter(!str_detect(ngram, stopwords))
...
export_word_embeddings <- function(dir, view_most_similar) {
  ...
  files <- list.files(path = dir, pattern = "congressional_speeches_climate_change_and_global_warming_tokens_", full.names = TRUE)
  ...
  for (file in files) {
    ...
    vocab_list = list(data_decade_subset$ngram)
    ...
    fname <- paste0(dir, "/", target_dir, "/", "congressional_speeches_climate_change_and_global_warming_word_vectors_", first_year_label, "_", last_year_label, ".txt")
...
z <- '/scratch/group/pract-txt-mine/congress_climate/'
export_word_embeddings(z, view_most_similar = F)
