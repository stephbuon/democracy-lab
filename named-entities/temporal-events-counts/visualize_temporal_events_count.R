...
ggplot(subset, aes(x = century, 
                   y = scholar_assigned_date, 
                   color = n)) + #, 
                   #label = paste0(entity, ' (', scholar_assigned_date, ')') )) + 
  scale_color_viridis(breaks = round, trans = "log", option = "B", discrete=F, direction=1) +
  geom_point(shape = 15, alpha = .3, aes(size = n*5)) +
  geom_text_repel(data=subset %>% 
                    group_by(entity) %>% 
                    arrange(desc(n)) %>% 
                    slice(2), 
                  aes(color = 1, x = century + 0.03, 
                      label = paste0(entity),
                      #label = paste0(entity, ' (', scholar_assigned_date, ')'), 
                      size = n*5), #size = rel(4), 
                  hjust=0) +
